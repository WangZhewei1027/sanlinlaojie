...
| 字号刻度 | `components/ui/typography.tsx` 的 `VARIANT_CLASSES` | 六级：30/24/18/16/14/12px，全部 rem 单位（见下文文字体系） |
...
- 已有：`npm run build`（TS strict）+ `npm run lint` + code review + 本文档/CLAUDE.md 对 AI 生效。
- **尚未落地**（见路线图）：fontSize 词汇表收窄、禁任意值/刻度外类的 lint 规则。在此之前，规范是"可遵守"而非"必遵守"，review 时需人工把关。
...
- `tone`：`default / subdued / critical`，对应语义色 token；不直接写 `text-muted-foreground`。
...
- 状态色（成功/警告）目前无 token，属体系缺口（见路线图），新增前不要再扩散 `text-green-600` 式写法。
...
1. **Token 词汇表收窄**：`theme.fontSize` 收敛到六级（现存 6 处 `text-xl` 迁移；首页 `text-4xl/5xl` 决定加 display 级或明文豁免营销路由）。
2. **lint 上牙齿**：禁 `text-[..px]` 任意值、禁刻度外字号、禁 Text 的 className 携带文字类、禁直接 import `@radix-ui/*`。
3. **状态色 token**：`--success` / `--warning`（含暗色），tone 扩展，迁移现存约 50 处 `text-red/green/amber-*`。
4. **Text 小修**：tone 默认改继承（有色表面可用）、`alignment` 改 `text-start/end` 逻辑属性、CardTitle 移除 `tracking-tight`。
